...
const NotificationItem = ({ data, onRead }: { data: INotification, onRead: (id: string) => void }) => {
  ...
  return (
    ...
    >
      ...
      {data.targetImage && <img src={data.targetImage} className={`w-10 h-10 rounded-md object-cover shrink-0 border ${isDark ? 'border-gray-700' : 'border-gray-200'}`} alt="target" />}
